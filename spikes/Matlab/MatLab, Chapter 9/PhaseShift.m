...
Stim = input('Stimulating pulse amplitude (0-2): ');
Pstart = input('Time (ms) for pulse = ');
whitebg('w');
T1 = clock;
...
SP = [43 43 43 43];
figure(1), ZA = plot(Time, 100 * X(1, :), 'r-', Ptime, Pulse, 'b-', NX, SP, 'r*');
axis([0, Final_Time, -90, 50]); xlabel('Time (ms)'); ylabel('V (red) & Pulse (blue)');
set(ZA, 'LineWidth', 2); title('Phase Resetting (normal spike times at dots)');
VV = -0.9:0.01:1.5;
