...
Stim2 = input('Stimulating current strength, neuron 2 (blue) (0-2): ');
ES = input('Excitatory synaptic conductance factor (0-10): ');
...
Leng = length(Rates);
Red_Rate = mean(Rates(round(Leng / 2):Leng))
...
Leng = length(BRates);
Blue_Rate = mean(BRates(round(Leng / 2):Leng))
disp(Blue_Rate)
