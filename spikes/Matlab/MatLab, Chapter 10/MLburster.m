...
for T = 2:Last;
    ...
    for rk = 1:4 %Fourth Order Runge-Kutta
        ...
        K(1, rk) = DT / Tau * (-1 / (1 + exp(- (XH(1) + 0.01) / 0.075)) * (XH(1) - 1.0) - 2 * XH(2) * (XH(1) + 0.7) - 0.1 * XH(3) * (XH(1) - 2.0) - XH(4) * (XH(1) + 0.7) - 0.5 * (XH(1) + 0.5) + Stim);
        K(2, rk) = DT * 0.4 * (-XH(2) + 1 / (1 + exp(- (XH(1) - 0.1) / 0.07)));
        K(3, rk) = DT / 10 * (-XH(3) + 1 / (1 + exp(- (XH(1) + 0.3) / 0.01)));
        K(4, rk) = DT / 50 * (-XH(4) + XH(3));
...
figure(1), ZA = plot(Time, X(1, :), 'r-'); set(ZA, 'LineWidth', 2); axis([0 500 -0.5 0.3]);
