...
Final_Time = 2000; %Final time value for calculation
...
for T = 2:Last;
    ...
    for rk = 1:4 %Fourth Order Runge-Kutta
        ...
        K(1, rk) = DT / Tau * (-1.0836 * (XH(1) - 0.48) - 26 * XH(2) * (XH(1) + 0.95) - 1.93 * XH(3) * (1 - 0.5 * XH(4)) * (XH(1) - 1.4) - 3.25 * XH(4) * (XH(1) + 0.95));
        K(2, rk) = DT / TauR * (-XH(2) + 1.29 * XH(1) + 0.79 + 3.3 * (XH(1) + 0.38) ^ 2);
        K(3, rk) = DT / TauC * (-XH(3) + Xgain * (XH(1) + 0.86) * (XH(1) + 0.84));
...
figure(1), ZA = plot(Time, 100 * X(1, :), 'r-'); set(ZA, 'LineWidth', 2); axis([0, Final_Time, -100, 20]);
xlabel('Time (ms)'); ylabel('Potential (mV)');
VV = -0.9:0.01:1.5;
DVdt = -0.5 * ((1.37 + 3.66 * VV + 2.6 * VV .^ 2) .* (VV - 0.48)) ./ (VV + 0.95);
