...
DT = 0.04; %Time increment as fraction of time constant
...
for T = 2:Last;
    ...
    for rk = 1:4 %Fourth Order Runge-Kutta
        ...
        K(1, rk) = DT / Tau * (- (17.81 + 47.71 * XH(1) + 32.63 * XH(1) ^ 2) * (XH(1) - 0.55) - 26 * XH(2) * (XH(1) + 0.92) - 0.54 * XH(3) * (XH(1) + 0.92) + Stim);
        K(2, rk) = DT / TauR * (-XH(2) + 1.35 * XH(1) + 1.03);
        K(3, rk) = DT / TauH * (-XH(3) + 9.3 * (XH(1) + 0.70));
...
figure(1), ZA = plot(Time, 100 * X(1, :), 'r-'); set(ZA, 'LineWidth', 1); axis([0 Final_Time -100 50]);
VV = -0.9:0.01:1.5;
